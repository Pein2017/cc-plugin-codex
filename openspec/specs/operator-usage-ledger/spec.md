...
- **WHEN** a fork rollout contains an in-window CC event without a non-empty call ID
...
- **THEN** the report records the unresolved file and rejects all of its candidate CC events rather than guessing which are imported
