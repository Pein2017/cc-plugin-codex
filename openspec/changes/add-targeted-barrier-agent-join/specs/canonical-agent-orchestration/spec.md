...
- **WHEN** `targets` contains one Agent whose active turn is snapshotted at wait entry
...
- **WHEN** `targets` contains multiple valid Agents with concrete snapshotted jobs
...
#### Scenario: Caller combines targets and progress wakeup
- **WHEN** a model-facing call supplies both `targets` and `wake_on_progress: true`
