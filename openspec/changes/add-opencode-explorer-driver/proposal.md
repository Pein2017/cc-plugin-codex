...
Lifecycle ordering: this change depends on exact candidate acceptance of `rename-to-codex-harnessdock` and `generalize-multi-harness-agent-control-plane`; their installed cutover/live witnesses may remain deferred. It is the sole change authorized to activate v3 public Agents and the eight-operation multi-Harness generation. Before implementation, re-run `openspec list` and rebase copied public-schema/receipt/card/usage/wait requirements against any accepted/archived `add-targeted-barrier-agent-join`, `expose-actionable-agent-blocking`, `improve-agent-card-and-usage-receipts`, `replace-wait-polling-with-event-wakeup`, and `harden-native-background-task-completion` result. After its candidate code/tests/review pass, stop at the consolidated install/data-cutover/Codex-restart boundary. Phase R physical production-source/deployment rename follows loaded release acceptance and precedes a third Harness.
