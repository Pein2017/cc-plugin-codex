...
- Add a durable root-scoped Agent Thread registry with flat stable path, root-unique name, description, distinct active/latest job pointers, exact Claude session, lifecycle status, and explicit continuation state.
- Add an Agent-level durable mailbox whose entries move through `queued`, `assigned(jobId)`, `dispatched`, and `acknowledged` independently of any one job process.
...
- Preserve Agent identity, mailbox, and latest Claude session pointer independently from the bounded 100 terminal job receipts per root; terminal job receipts remain the fact source and Agent/inbox records are rebuildable projections.
...
- Adopting a foreign Terminal-created Claude session is outside v0.2 and requires a separate future OpenSpec change; this release creates and resumes only plugin-owned Agent sessions.
...
- Adding nested topology, tags, templates, automatic nicknames, archive, close, delete-history, or permanent resident workers.
- Adopting or importing Terminal-created Claude sessions.
