...
- Scope every Agent to one Codex root as a logical default-isolation boundary while retaining cross-root diagnostics only in an operator CLI.
...
- flat stable `/root/<task_name>` `path`;
- user-supplied `name` and optional `description`;
- `rootThreadId` (identical to hardened `ownerRootId`) and canonical `workspaceRoot`;
- `activeJobId` or null, plus `latestJobId` or null;
- validated `claudeSessionId` or null;
- `status`: `pending_init`, `running`, `completed`, `interrupted`, or `errored`;
- `continuation`: `exact_session`, `safe_fresh`, or `blocked`, with evidence;
- creation/update timestamps and latest completion sequence.

The first job and every later follow-up point back to the Agent. Agent continuity reads the registry's current session pointer, not an arbitrary old job file. `activeJobId` is cleared after terminal publication while `latestJobId` remains as a diagnostic pointer. Activation-blocked is expressed by `continuation=blocked`, never as a lifecycle status. Names are unique within a root and duplicate spawn fails rather than silently suffixing or resuming another Agent.
...
`rootThreadId` is the already-hardened `ownerRootId` injected by the bootstrap and is mandatory for normal operations. It is a logical boundary against accidental cross-root orchestration, not a cryptographic authorization claim. Model-facing calls cannot override it. `list_agents` returns only the current root. A separate operator CLI provides explicit redacted `--all` diagnosis and does not grant cross-root messaging, follow-up, interruption, waiting, or acknowledgement.
...
| List | Root tree with optional `path_prefix` | Flat `/root/<task_name>` topology with exact targeting and prefix-filtered listing; all logical nonresident history remains listed; cross-root `--all` exists only in operator CLI |
...
`send_message` writes an Agent-mailbox entry and delivers it to an active turn when possible; if no turn is active, it remains queued without starting one. Mailbox entries move atomically through `queued -> assigned(jobId) -> dispatched -> acknowledged`. `followup_task` uses the same durable message path but also guarantees work: it starts an exact-session or receipt-proven safe-fresh turn when the Agent is terminal, atomically assigns queued entries to that job, or makes the new message available promptly to the active turn. One Agent has at most one active job, while multiple Agents under the same root may run concurrently subject to evidence-derived capacity policy.
...
- [A registry update, Agent mailbox transition, completion projection, and job transition can crash between writes] → Treat the internal terminal job receipt as the fact source; use deterministic linkage and startup reconciliation to rebuild Agent and inbox projections; never infer session ownership from name alone.
...
- [A direct Terminal session is outside plugin root ownership] → Reject adoption in v0.2 and require a separate future OpenSpec before adding any handoff workflow.
...
2. Add the versioned Agent registry, logical root/name indexes, Agent mailbox, Claude session-root bindings, atomic transitions, and job/completion reconciliation.
