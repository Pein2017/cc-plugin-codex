...
Lifecycle ordering: the exact `rename-to-codex-harnessdock` candidate must first pass its checkout-level implementation, full tests, and review; its installed cutover may remain deferred. This change must then be candidate-accepted at an exact tree before `add-opencode-explorer-driver`, but both changes may execute sequentially in the same task/worktree with separate ledgers, review checkpoints, and commits. Before implementation, re-run `openspec list` and rebase every copied MODIFIED/RENAMED requirement and overlapping file plan against any accepted/archived `add-targeted-barrier-agent-join`, `expose-actionable-agent-blocking`, `improve-agent-card-and-usage-receipts`, `replace-wait-polling-with-event-wakeup`, and `harden-native-background-task-completion` result. The last change remains the owner of stronger Claude terminal settlement: until it proves a stronger fact, the Claude adapter maps ambiguous native owned work to `unknown` and retains leases rather than claiming completion.
