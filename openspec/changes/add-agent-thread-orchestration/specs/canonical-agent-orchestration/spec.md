...
#### Scenario: Foreign session adoption is requested by the model
- **WHEN** model-facing spawn includes an existing Claude session ID
- **THEN** spawn rejects it and points to the separate user-authorized operator workflow

### Requirement: send_message never activates an idle Agent
`send_message` SHALL durably deliver to an active Agent turn when possible and SHALL queue the message without starting a new turn when the Agent is terminal.
...
- **THEN** it is retained with a `queued_no_turn` receipt and no Claude process starts

#### Scenario: Agent is activation-blocked
- **WHEN** an errored Agent has neither a resumable session nor receipt-proven safe fresh retry
...
`followup_task` SHALL make the message available to an active Agent promptly or start a new exact-session turn when the Agent is terminal and resumable.
...
`list_agents` SHALL accept only the canonical optional `path_prefix` and return matching current-root Agent identities, descriptions, lifecycle states, current job pointers, Claude session availability, and unread completion summaries without acknowledging them. Cross-root `--all` SHALL exist only in the separate operator CLI.
