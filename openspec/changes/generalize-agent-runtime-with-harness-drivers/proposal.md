...
- Introduce explicit version-2 Agent and job state capable of naming a Harness, immutable model route, neutral native-session reference, driver version, capability snapshot, and Harness-scoped session lease. Existing version-1 state is interpreted only as Claude Code, remains readable, and is never rewritten while owned by an active version-1 worker.
