...
- `runtime/execution-profile.mjs` is the sole owner of Claude CLI overrides. `terminal-parity` always adds `IS_SANDBOX=1` and adds `--dangerously-skip-permissions` only for explicit write intent; read intent keeps native Claude permissions. It must not acquire an implicit model, effort, settings, tool, MCP, or prompt override except the OpenSpec-owned bounded delegation envelope and leaf `Agent` denial.
...
- Keep the seven Agent skills under `plugins/cc-for-pein/skills/`, `runtime/`, package/manifest metadata, and `tests/runtime*` in sync. Old upstream material is not a compatibility contract.
- Run `npm run check` before merging.
- For releases, update `CHANGELOG.md` and the base version in `package.json`; the cachebuster refresh derives `plugins/cc-for-pein/.codex-plugin/plugin.json` from that single source.
