...
function rejectPublicContextOverrides(rawArgv) {
  ...
  if (forbidden) {
    throw new Error(
      `Unsupported model-facing option ${forbidden}. HarnessDock lifecycle calls inherit the Codex working directory and use the Plugin's fixed environment.`
    );
  }
}
...
function resolveCheckout() {
  ...
  if (!existing(cli) || !envFile || !existing(manifest)) {
    throw new Error(`Fixed HarnessDock runtime checkout is invalid: ${checkout}`);
  }
  const plugin = JSON.parse(fs.readFileSync(manifest, "utf8"));
  if (plugin.name !== "codex-harnessdock") {
...
function main() {
  ...
  child.on("exit", (code, signal) => {
    if (signal) {
      process.stderr.write(`HarnessDock runtime exited from ${signal}.\n`);
      process.exitCode = 1;
      return;
    }
