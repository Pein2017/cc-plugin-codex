...
You get nine commands (`$cc:review`, `$cc:adversarial-review`, `$cc:rescue`, `$cc:status`, `$cc:result`, `$cc:steer`, `$cc:interrupt`, `$cc:cancel`, `$cc:setup`) that launch tracked Claude Code work, manage lifecycle and ownership, and surface results back into Codex.

That includes:
- Direct main-thread launch for rescue tasks, plus Codex subagent orchestration for background review flows
- Session-scoped tracked jobs with status, result, steering, interrupt, and cancel commands
...
| `$cc:steer` | Redirect an active job, or explicitly follow up on a finished one |
| `$cc:interrupt` | Gracefully stop a job while preserving its Claude session |
| `$cc:cancel` | Destructively terminate an active background job |
...
Background rescue launches the tracked companion worker directly from the current Codex thread. The launch returns a durable job id immediately; no disposable forwarding subagent is needed.
...
```text
$cc:steer task-abc123 prefer the smaller fixture
$cc:steer --follow-up task-abc123 verify the remaining edge case
```

Active steering is persisted before the command returns. It is delivered through Claude's native stream input at the next safe boundary, or after the next recovery attempt if the connection is currently down. `--follow-up` is required for completed or interrupted jobs and creates a new tracked job on the exact prior Claude session.

### `$cc:interrupt`

```text
$cc:interrupt task-abc123
```

Interrupt sends SIGINT without escalating to SIGKILL. The job becomes `interrupted`, retains partial output and its exact Claude session id, and can continue with `$cc:steer --follow-up`.

### `$cc:cancel`

```text
$cc:cancel task-abc123              # destructively terminate a queued/running job
```
...
1. **Queued → Running → Completed** — healthy jobs progress through states automatically. Recovery adds a visible `reconnecting` phase; control paths distinguish `interrupted`, `cancelled`, and failures.
2. **Direct rescue background flow** — rescue uses the companion's detached tracked worker directly. Background review and adversarial review retain their Codex-managed review flow.
...
4. **Durable recovery receipts** — partial output, reconnect attempts, exact Claude session ownership, and steering acknowledgements remain inspectable through status and result.
5. **Session ownership** — jobs stay attached to the user-facing parent Codex session, so plain `$cc:status`, `$cc:result`, and resume-candidate detection still follow the parent thread.
...
# ... keep working ...
$cc:steer task-abc123 focus on the allocator first
...
- Rescue runs in a plugin-owned detached worker with durable state, exact-session recovery, and a steering mailbox.
- Background reviews retain their review-specific Codex orchestration.
...
| **Runtime** | Codex app-server + broker | Supervised `claude -p` stream attempts per logical job |
...
- **Resilient logical sessions** — transport closures resume the exact emitted Claude session with bounded backoff, without replaying the original task.
- **Live steering and graceful interrupt** — durable stdin steering, explicit exact-session follow-up, and SIGINT-based interruption remain distinct from destructive cancel.
...
Develop and test directly from a clone or worktree:

```bash
npm ci
node scripts/claude-companion.mjs task --background --fresh "inspect the current checkout"
node scripts/claude-companion.mjs status --all
```

The checkout is the only source and runtime dependency in this mode. Do not point development at a versioned directory under Codex's plugin cache. Current Codex plugin installation materializes a cache copy, so install only after development is complete and treat that copy as an integration artifact, not editable source.
