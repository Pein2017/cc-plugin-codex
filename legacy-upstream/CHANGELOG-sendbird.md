...
- Supervise each tracked rescue as one logical Claude session across bounded native stream-json subprocess attempts, resume the exact emitted session after transient transport closure, and surface recovery/partial/runtime receipts.
- Add durable live `$cc:steer`, exact-session follow-up, and graceful `$cc:interrupt` controls while keeping destructive cancel separate.
- Launch rescue background work directly from the user-facing Codex thread without a disposable forwarding subagent, and document checkout-only local development that does not use Codex's versioned plugin cache as source.
